// The masthead logo: a wax-seal emblem carrying a debossed "TL" monogram, next
// to "The Ledger" set in IM Fell English with a brushed-brass gilt fill. The
// seal doubles as the app's standalone mark (public/seal.svg, also the favicon).
// Everything is live SVG: real <text> for the wordmark (accessible + crisp at any
// size, no font baked to paths) and static shapes for the seal.
...
export function buildTitle(): void {
  ...
  const fit = () => {
    const word = svg.querySelector('.lg-word') as SVGGraphicsElement | null;
    if (!word) return;
    try {
